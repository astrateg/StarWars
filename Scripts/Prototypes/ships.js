define(['Modules/game', 'Modules/ship'], function (GAME, SHIP) {
    ...
    Ship.prototype.Show = function () {
        ...
        if (this.ID == SHIP.MyShip.ID) {    // Если наш корабль - подсвечиваем его
            ...
            currentShipHPValue.innerText = Math.round(this.HP);
        ...
        var currentShipMPValue = document.getElementById('CurrentShipMPValue');
        currentShipMPValue.innerText = Math.round(this.MP);
        currentShipMPValue.style.width = (this.MP / this.MaxMP) * 124 + "px";    // 124 = 128 - 2 - 2 (размер рисунка с кораблем минус границы 2px)
        currentShipMPValue.style.backgroundColor = color.allShipMP;
