...
require(['jquery', 'Modules/game', 'Modules/requests', 'Modules/ship', 'Modules/space', 'Modules/utils', 'Prototypes/ships', 'Prototypes/bombs'],
    function ($, GAME, REQUESTS, SHIP, SPACE, UTILS, Ship, Bomb) {
    ...
    // Ship construstor (OLD): Ship(id, name, type, maxHP, HP, X, Y, speed, angle, angleSpeed, size, image, vectorMove, vectorRotate, delay, shoot, kill, death)
    // delay, shoot, kill, death - not required parameters

    // Ship construstor (NEW): Ship(id, name, type, state, maxHP, HP, X, Y, angle, size, image, vectorMove, vectorRotate, kill, death)

    function LoadShips(ships, id) {
        if (ships[0] != null) {
            for (var i = 0; i < ships.length; i++) {
                // Если корабль неактивен, пропускаем его - не показываем на карте и на левой панели
                //if (ships[i].State == "Inactive") {
                //    continue;
                //}
                ...
                var otherShip = new Ship(ships[i].ID, userName, ships[i].Type, ships[i].State, ships[i].MaxHP, ships[i].HP, ships[i].X, ships[i].Y, ships[i].Angle, ships[i].Size, imageIndex, ships[i].VectorMove, ships[i].VectorRotate, ships[i].Kill, ships[i].Death);
            ...
            if (data.ships.length == 0) {
                ...
                for (var i = 0; i < serverShipsCount; i++) {
                    ...
                    for (var j = 0; j < clientShipsCount; j++) {
                        ...
                        SHIP.Ships[j].MaxHP = data.ships[i].MaxHP;
